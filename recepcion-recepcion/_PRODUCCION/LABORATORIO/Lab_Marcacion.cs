...
{   
    public partial class Lab_Marcacion : Form
    {
        ...
        public static string estado_laboratorio_env;
        string codigotipo;
      //  string ORDEN;
        String fecha;
        String fecha_up;
       ...
       public static string ORDEN;
        ...
        private bool codigo_barras_job()
        {
            //busva si la orden tiene ingresado el numero de job que genera el lms...
            int contar = 0;
            try
            {
                con.conectar("LESA");
                SqlCommand cmd = new SqlCommand("SELECT [LMS] FROM [LDN].['" + Exten_ + "'].[PEDIDO_DET] WHERE COD_ORDEN = '" + ORDEN + "' GROUP BY LMS", con.cmdls);
                //cmd.Parameters.AddWithValue("NUM_DOC_PREIMP", Convert.ToInt32(orden));
        ...
        private bool con_estado_laboratorio()
        {
            con.conectar("LESA");

            SqlCommand cmd = new SqlCommand("SELECT COUNT( [ESTADO_LAB]) FROM [LDN].[LDN].[PEDIDO_ENC] WHERE ESTADO_LAB = 'LABORATORIO' AND COD_ORDEN = '" + ORDEN+"'", con.cmdls);
      ...
      /*  private void update_fecha_estado()
        ...
        private string obtener_idlab(string lab, string sig_lab)
        {
           // string sig_lab;
            ...
            //string Repro;
            con.conectar("NV");
            SqlCommand comand7 = new SqlCommand("SELECT [ID_ETAPA],[NOMBRE] FROM [LDN].[ETAPAS_LAB] WHERE NOMBRE = '"+lab+"'", con.cmdnv);
            SqlDataReader dr7 = comand7.ExecuteReader();
