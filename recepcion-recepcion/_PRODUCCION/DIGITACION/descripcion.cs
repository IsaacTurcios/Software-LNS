...
namespace LND
{
    public partial class descripcion : Form
    {
        ...
        private void descripcion_Load(object sender, EventArgs e)
        {
            ...
            Symbol_money = LOGIN.simbol_money;

            tbl_cod_emp.Columns.Add("COD_EMPRE", typeof(string));
            tbl_cod_emp.Columns.Add("CLIENTE", typeof(string));
            tbl_cod_emp.Columns.Add("NUM_EMP", typeof(string));
            tbl_cod_emp.Columns.Add("LISTADO", typeof(string));
            tbl_cod_emp.Columns.Add("VENDEDOR", typeof(string));
           //////tbl_cod_emp.Rows.Add(cod_pais_log);

            //colocando  simbolos segun la moneda del pais.
            label9.Text = Symbol_money;
            label21.Text = Symbol_money;
            label72.Text = Symbol_money;
            label94.Text = Symbol_money;
            label28.Text = Symbol_money;
            label77.Text = Symbol_money;
            label79.Text = Symbol_money;
            label99.Text = Symbol_money;
            label82.Text = Symbol_money;
            label84.Text = Symbol_money;
            label29.Text = Symbol_money;

            //Deshabilitando los campos de formula:
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (existe_pedido(cod_cliente_des))
            {
                ...
                if (dialogResult == DialogResult.Yes)
                {
                    if (day == null)
                    {
                        MessageBox.Show("no selecciono una fecha difereente de hoy no se guardo el pedido correctamente por favor verifique..");
                    }
                    else
                    {
                    ...
                    case 6:  //agregar
                        {
                            ...
                            for (int p = 0; p < tbl_cod_emp.Rows.Count; p++)
                            {
                                ...
                                emp_ = Convert.ToString(rp["COD_EMPRE"]);
                                codigo_ = Convert.ToString(rp["NUM_EMP"]);
                                Cod_clie_tb = Convert.ToString(rp["CLIENTE"]);
                                vendedor_ = Convert.ToString(rp["VENDEDOR"]);
                                name_listado = Convert.ToString(rp["LISTADO"]);

                                #region validacion de campos
                                ////verificando el codigo del cliente en la tabla....
                                if (Cod_clie_tb == null || Cod_clie_tb == string.Empty || Cod_clie_tb == "")
                                {
                                    Cod_clie_tb = Cod_clie;
                                }
                                else
                                {
                                    Cod_clie = Cod_clie_tb;
                                }

                                /////verificando el vendedor de la tabla.....
                                if (vendedor_ == null || vendedor_ == string.Empty || vendedor_ == "")
                                {
                                    vendedor_ = COD_VEND;
                                }
                                else
                                ...
                                /////verifiacando el nombre del listado...(variable Listado)
                                if (name_listado == null || name_listado == string.Empty || name_listado == "")
                                {
                                    name_listado = Listado;
                                }
                                else
                                {
                                    Listado = name_listado;
                                }
                                #endregion validacion de campos

                                if (tbl_cod_emp.Rows.Count == 1) // cuando la tabla solo tiene un valor de uno.. a una empresa
                                {
                                   insercion_completa(codigo_);
                                }
                                else
                                { 
                                     insercion_completa(codigo_);
...
#region modificado                         
                            ...
                            //        }
                            //        else//si es otro pais diferente de el salvador
                            //        {
                            //            //INGRESANDO PEDIDOS...
                            ...
                            //            cargar_precios();
                            //            buscando_id_listado_precios(Listado);
                            //         //   select_info_cliente();
                            //            ///listado_precios(name_listado);
                            //            ///-----///
                            //            insert_encabezado(codigo_); // TABLA ENCA
                            //            insertar_detalle_sql(codigo_); // table ENC_PED_CAMPL
        ...
        private void insercion_completa(string codigo_pais_logueado)
        {
            #region ingresando pedidos...
            if (cod_pais_log == codigo_)
            {
                ///este si entrara a sae por ser local..
                //INGRESANDO PEDIDOS...
            ...
            else
            {
                    ...
                    /////no entrara a sae por ser extranjero...
                    //INGRESANDO PEDIDOS...
                    ...
                    {
                        ///agrega el numero de sae en uno de los campos de complemento con el numero del pedido que se hiso en el otro pais..
                        update_pdguate(cod_orden, orden_de_guatemala);
        ...
        private void update_pdguate(string codigo_orden, string codigo_dguate)
        {
            ...
            SqlCommand cmd = new SqlCommand("SELECT COD_ORDEN, [PEDIDO_SAE] FROM ["+Exten_+"].[PEDIDO_ENC]  where COD_ORDEN = '" + codigo_dguate + "'", cnx.cmdnv);
            SqlDataReader dr = cmd.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void cmbList_precio_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            ///segun la descripcion ir  sacar el codigo del listado seleccionado

            buscando_id_listado_precios(Listado);
           // listado_precios();
        }

        private void listado_precios(string codigo_listado)
        {

            DataView dtv = new DataView(precios);
            dtv.RowFilter = "CVE_PRECIO = '" + codigo_listado + "'";
        ...
        private void insertar_estado_bodega_fecha(string codigo_emp)
        {
            ...
            cmd.Parameters.AddWithValue("@FECHA_MOD" ,Convert.ToDateTime(fecha_mod));
        ...
        private int orden_ultimo()
        {
            ...
            else if (codigo_ == "3")
            {
                ...
                cnx.conectar("NV");
                SqlCommand cmd = new SqlCommand("select top 1 [ID_PED] from [LDNHN].[PEDIDO_ENC] order by ID_PED desc", cnx.cmdnv);
                tear = Convert.ToInt32(cmd.ExecuteScalar());
                cnx.Desconectar("NV");
        ...
        {
            ///usuario del pais que ingreso:
            string nummero_bd = LOGIN.emp_;
            string cliente_envio ="";
            string vendedor_envio = "";
            string listado_envio = "";

            if (cod_cliente_des == null || cod_cliente_des == string.Empty || cod_cliente_des == "")
            {
                MessageBox.Show("No tiene cliente");
            }
            else
            {
                ///lo ingresa por defecto a la tabla ...
               tbl_cod_emp.Clear();
               tbl_cod_emp.Rows.Add(nummero_bd, cod_cliente_des, cod_pais_log, null, null);
               pais_ = null;
               cod_pais_ = null;
               cliente_envio = null;
               vendedor_envio = null;
               listado_envio = null;
                ...
                cnx.conectar("NV");
                SqlCommand clp = new SqlCommand("SELECT EMP.EMPRESA ,[ID_PAIS_ENVIO] ,[ID_SUCURSAL_ENVIO],EMP.ID_BD ,[ID_PAIS_CLIENTE_ENVIO] ,[ID_PAIS_LISTADO_PRECIOS], [VENDEDOR] FROM [LDN].[LDN].[CONTROL_CLIENTE_SUCURSAL] AS CTR LEFT JOIN [LDN].[LDN].[EMPRESAS] AS EMP ON CTR.ID_PAIS_ENVIO = EMP.ID_EMP WHERE ID__PAIS_INGRESADO='" + cod_pais_log + "' AND EMP.EMPRESA = '" + comboBox4.Text + "'  AND ESTATUS = 'A'", cnx.cmdnv);
                SqlDataReader dr = clp.ExecuteReader();
                while (dr.Read())
                {
                    pais_ = Convert.ToString(dr["ID_BD"]);
                    cod_pais_ = Convert.ToString(dr["ID_PAIS_ENVIO"]);
                    cliente_envio = Convert.ToString(dr["ID_PAIS_CLIENTE_ENVIO"]);
                    vendedor_envio = Convert.ToString(dr["VENDEDOR"]);
                    listado_envio = Convert.ToString(dr["ID_PAIS_LISTADO_PRECIOS"]);
                }
                dr.Close();

                if (cod_pais_ == null || cod_pais_ == "" || cod_pais_ == string.Empty)
                {
                    MessageBox.Show("Usted no tiene permisos para enviar este pedido al pais seleccionado");
                }
                else
                {
                    ...
                    {
                        tbl_cod_emp.Clear();
                        tbl_cod_emp.Rows.Add(nummero_bd, cod_cliente_des, cod_pais_log, null, null); //adicionando el codigo de pais del que se encuentra logeado...
                    ...
                    else
                    {
                        ...
                        {
                            tbl_cod_emp.Rows.Add(pais_, cliente_envio, cod_pais_, listado_envio, vendedor_envio);
                        }
                        else
                        {
                            tbl_cod_emp.Clear();
        ...
        private void buscando_id_listado_precios( string Listado_combo)
        {
            cnx.conectar("NV");
            SqlCommand clp = new SqlCommand("SELECT [CVE_PRECIO],[DESCRIPCION]  FROM [SAE50Empre"+emp_+"].[dbo].[PRECIOS"+emp_+"] WHERE DESCRIPCION = '" + Listado_combo + "'", cnx.cmdnv);
            SqlDataReader dr = clp.ExecuteReader();
            while (dr.Read())
            {
                cod_listado_precio = Convert.ToString(dr["CVE_PRECIO"]);
             
            }
            dr.Close();

            listado_precios(cod_listado_precio);
