...
namespace LND
{
    public partial class Base : Form
    {
        ...
        private void Base_Load(object sender, EventArgs e)
        {
          ...
          fechita = DateTime.Now.ToString("yyyy/MM/dd");
            emp_ = LOGIN.emp_;
            Exten_ = LOGIN.slg_;
        ...
        private void descripciones_art()
        {
            ...
            if (articulos.Rows.Count == 1)
            {
                ...
                linea_producto_venta = Convert.ToString(dr["LIN_PROD"]);
            ...
            else if (articulos.Rows.Count == 2)
            {
               ...
               // DataRow da = articulos.Rows[2];
                ...
                textBox3.Text = Convert.ToString(dr["OIES"]);
                textBox6.Text = Convert.ToString(dr["OICIL"]);
                label5.Text = Convert.ToString(ds["DESCR"]);
                linea_producto_venta = Convert.ToString(dr["LIN_PROD"]);
                //aro o servicio(AR)segun lo que tenga la orden
            ...
            else if (articulos.Rows.Count == 3)
            {
                ...
                label35.Text = Convert.ToString(da["DESCR"]);
                linea_producto_venta = Convert.ToString(dr["LIN_PROD"]);
                //servicio o aro segun lo que tenga la orden
            }
            ...
            if (linea_producto_venta == "7") //--Terminados
            {
                ///termiando check 2
                this.checkBox2.Enabled = true;
                this.checkBox1.Enabled = false;
            }
            else if (linea_producto_venta == "2") // --Proceso
            {
                ///procesp check 1
                     ...
                     if (dialogResult == DialogResult.Yes)
                     {
                       /* if (codigo_barra == null || codigo_barra == string.Empty || codigo_barra == "")
                        ...
                        {*/
                            ...
                            MessageBox.Show("Estimado/a la asignacion de las bases, se realizo exitosamente, le pedimos verifiacar.   ", "Mensaje de Alerta... ", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        /*
                        }*/
                    }
                     else { 
                ...
                else if (checkBox1.Checked == true) ////proeducto de porceso
                {
                    ...
                    else
                    {
                        ...
                        if (dialogResult == DialogResult.Yes)
                        {
                            ...
                            tabla_productos(); ///inserta los productos
                            insertar_laboratorio();        /// inserta al laboratorio    
                            insertar_estado_laboratorio(); /// cambia de estado
                            llenar_observaciones(); /// ingresa observaciones..
                            update_numero_job(JOB); ///  update para colocar el lms
                        }
                        else
                        {
                        }
        ...
        private void tabla_productos()
        {
            ...
            for (int p = 0; p < productos.Rows.Count; p++)
            {
                ...
                try
                {
                    ...
                    cmd_b.Parameters.AddWithValue("@ESTATUS", "A");
                    cmd_b.ExecuteNonQuery();
                    cnx.Desconectar("NV");

                    ///Movimiento de entrada a la bodega de produccion...
                   /// movimientos(num_orden, codigo_, cantidad_, fecha_ing);

                }
                catch (Exception e)
                {
                    MessageBox.Show("Alerta al realizar asignacion de base...", e.ToString());
                }
      ...
      /* private void insertar_detalle_clic()
        ...
        private void update_numero_job(string job)
        {
            cnx.conectar("NV");
       ...
       /* private void insertar_detalle_base()
        ...
        private void llenar_marca_terminado()
        {
            ...
            cmd = new SqlCommand("SELECT CAMPLIB33  FROM [SAE50Empre"+emp_+"].[dbo].[INVE_CLIB"+emp_+"] cl left join[SAE50Empre"+emp_+"].[dbo].[INVE"+emp_+"] iv on cl.CVE_PROD = iv.CVE_ART  WHERE iv.LIN_PROD = '"+linea_producto+"' AND iv.STATUS = 'A' AND CAMPLIB40 = 'INSUMO DE PRODUCCION' AND CAMPLIB29 = '"+ campo_libre + "' AND CAMPLIB30 = '"+label17.Text+"'  group by CAMPLIB33 order by CAMPLIB33 desc", cnx.cmdls);
            dr = cmd.ExecuteReader();
            while (dr.Read())
            {
                comboBox4.Items.Add(dr["CAMPLIB33"]);
        ...
        private void confirmar_base()
        {
            ...
            {
                ///base de proceso
                consulta = "SELECT DESCR, CVE_ART ,COSTO_PROM FROM [SAE50Empre06].[dbo].[INVE_CLIB06] cl left join [SAE50Empre06].[dbo].[INVE06] iv on cl.CVE_PROD = iv.CVE_ART  WHERE iv.LIN_PROD ='" + linea_producto+"' AND iv.STATUS = 'A' AND CAMPLIB40 = 'INSUMO DE PRODUCCION' AND CAMPLIB29 = '"+campo_libre+"' AND CAMPLIB30 = '"+label17.Text+"' AND CAMPLIB33 = '"+comboBox4.Text+"' AND CAMPLIB15 = '"+comboBox3.Text+"'  AND CAMPLIB16 = '"+comboBox2.Text+"'";
        ...
        private void comboBox6_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            detalle_tabla.Clear();
            cnx.conectar("NV");
            SqlCommand sql = new SqlCommand("SELECT  [CVE_PROD] ,[CAMPLIB40] AS CATEGORIA ,[CAMPLIB30] AS MATERIAL ,[CAMPLIB1] AS TIPO_LENTE ,[CAMPLIB2] AS PROVEEDOR, [CAMPLIB32] AS TIPO_BASE FROM [SAE50Empre"+emp_+"].[dbo].[INVE_CLIB"+emp_+"] where  [CAMPLIB40] = 'INSUMO DE PRODUCCION' AND [CAMPLIB30] = '" + material_ + "' AND [CAMPLIB1] = 'TERMINADO' ");
            sql.Connection = cnx.cmdnv;
            SqlDataAdapter dr = new SqlDataAdapter(sql);
            dr.Fill(detalle_tabla);
            cnx.Desconectar("NV");
        ...
        private void comboBox4_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            /*    cmd1 = new SqlCommand("SELECT  INV.[CVE_PROD] , INVE.[DESCR] FROM [SAE50Empre"+emp_+"].[dbo].[INVE_CLIB"+emp_+"] as INV LEFT JOIN  [SAE50Empre"+emp_+"].[dbo].[INVE"+emp_+"] AS INVE ON INV.[CVE_PROD] = INVE.CVE_ART WHERE [CAMPLIB40] = 'INSUMO DE PRODUCCION' AND [CAMPLIB30] = '"+material_+"' AND [CAMPLIB1] = 'TERMINADO' AND [CAMPLIB32] = '"+comboBox4.Text+"'", cnx.cmdls); */
            cmd1 = new SqlCommand("SELECT [CAMPLIB15] FROM [SAE50Empre"+emp_+"].[dbo].[INVE_CLIB"+emp_+"] cl left join [SAE50Empre"+emp_+"].[dbo].[INVE"+emp_+"] iv on cl.CVE_PROD = iv.CVE_ART  WHERE iv.LIN_PROD ='" + linea_producto + "' AND iv.STATUS = 'A' AND CAMPLIB40 = 'INSUMO DE PRODUCCION' AND CAMPLIB29 = '" + campo_libre + "' AND CAMPLIB30 = '" + label17.Text + "' AND CAMPLIB33 = '" + comboBox4.Text + "'  group by [CAMPLIB15] order by [CAMPLIB15] desc", cnx.cmdls);

            dr1 = cmd1.ExecuteReader();
            while (dr1.Read())
            {
                comboBox3.Items.Add(dr1["CAMPLIB15"]);
        ...
        private void comboBox3_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            /*    cmd1 = new SqlCommand("SELECT  INV.[CVE_PROD] , INVE.[DESCR] FROM [SAE50Empre"+emp_+"].[dbo].[INVE_CLIB"+emp_+"] as INV LEFT JOIN  [SAE50Empre"+emp_+"].[dbo].[INVE"+emp_+"] AS INVE ON INV.[CVE_PROD] = INVE.CVE_ART WHERE [CAMPLIB40] = 'INSUMO DE PRODUCCION' AND [CAMPLIB30] = '"+material_+"' AND [CAMPLIB1] = 'TERMINADO' AND [CAMPLIB32] = '"+comboBox4.Text+"'", cnx.cmdls); */
            cmd1 = new SqlCommand("SELECT [CAMPLIB16] FROM [SAE50Empre"+emp_+"].[dbo].[INVE_CLIB"+emp_+"] cl left join [SAE50Empre"+emp_+"].[dbo].[INVE"+emp_+"] iv on cl.CVE_PROD = iv.CVE_ART  WHERE iv.LIN_PROD ='"+linea_producto+"' AND iv.STATUS = 'A' AND CAMPLIB40 = 'INSUMO DE PRODUCCION' AND CAMPLIB29 = '"+campo_libre+"' AND CAMPLIB30 = '"+material_+"' AND CAMPLIB33 = '"+comboBox4.Text+"' AND CAMPLIB15 = '"+comboBox3.Text+"'  group by [CAMPLIB16] order by [CAMPLIB16] desc", cnx.cmdls);

            dr1 = cmd1.ExecuteReader();
            while (dr1.Read())
            {
                comboBox2.Items.Add(dr1["CAMPLIB16"]);
        ...
        private void insertar_laboratorio()
        {
            ...
            {
                MessageBox.Show("Insertando al Laboratorio",ex.ToString());
            }
        ...
        private bool Buscar_codigo_barras_articulo(string codigo_articulo)
        {
            cnx.conectar("LESA");

            SqlCommand cmd = new SqlCommand("SELECT [CVE_ALTER]  FROM [SAE50Empre"+emp_+"].[dbo].[CVES_ALTER"+emp_+"] WHERE CVE_ART = '"+ codigo_articulo + "'", cnx.cmdls);
        ...
        private void Update_cvl_temporal(string codigo_barras_, string articulo_)
        {
                ...
                cnx.conectar("LESA");
                SqlCommand cmd = new SqlCommand("UPDATE [SAE50Empre" + emp_ + "].[dbo].[CVES_ALTER" + emp_ + "] SET CVE_ALTER = '" + codigo_barras_ + "' WHERE CVE_ART = '" + articulo_ + "'", cnx.cmdls);
                cmd.ExecuteNonQuery();
                cnx.Desconectar("LESA");
                MessageBox.Show(" El codigo de barras fue exitosamente agregado al producto... ", "Agregando codigo de barras", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            if (var_cod_base == null || var_cod_base == string.Empty || var_cod_base == "")
            {
                MessageBox.Show("Comentarle que no a seleccionado un articulo", "no tiene codigo", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                    if (Buscar_codigo_barras_articulo(var_cod_base))
                    {
                        ///si tiene codigo de barras...
                        MessageBox.Show(" Esstimado, comentarle que este producto ya tiene un codigo de barra establecido, le pido que verifique buscandolo... ", "COMENTARIO, Busqueda - Modificacion de bases", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    else
                    {
                        ///no se tiene el codigo de barras...
        ...
        private void movimientos(string orden, string clave, string cantidad, string fecha)
        {
            ...
            try
            {
                ...
                mv.Movimiento_Entrada(emp_, orden, clave, cantidad, fechita, fecha, 7, 12);
                mv.Movimiento_Salida(emp_, orden, clave, cantidad, fechita, fecha, 58, 1);
