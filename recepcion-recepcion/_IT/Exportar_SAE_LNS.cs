...
namespace LND
{
    public partial class Exportar_SAE_LNS : Form
    {
        ...
        private Boolean existe(string ped)
        {
            ...
            SqlCommand cmd = new SqlCommand("SELECT COUNT([PEDIDO_GUATE]) FROM [LDN].[PEDIDO_ENC] WHERE PEDIDO_GUATE = '"+ ped +"'", con.cmdnv);
        ...
        private Boolean existe_detalle(string gt, string linea)
        {
            ...
            SqlCommand cmd = new SqlCommand("SELECT COUNT(DET.COD_ORDEN) FROM [LDN].[PEDIDO_DET] AS DET LEFT JOIN [LDN].[PEDIDO_ENC] as EN ON DET.COD_ORDEN = EN.COD_ORDEN WHERE EN.PEDIDO_GUATE = '"+ gt +"' AND DET.[NUM_PAR] = '" + linea + "'", con.cmdnv);
        ...
        private Boolean existe_ped(string ord)
        {
            ...
            SqlCommand cmd = new SqlCommand("SELECT COUNT([PEDIDO_GUATE]) FROM [LDN].[PEDIDO_ENC] WHERE  [PEDIDO_GUATE] = '" + ord + "'", con.cmdnv);
        ...
        private string obtener_ord(string cod)
        {
            ...
            SqlCommand cmd = new SqlCommand("SELECT [COD_ORDEN] ,[PEDIDO_GUATE]  FROM [LDN].[PEDIDO_ENC]  WHERE [PEDIDO_GUATE] = '"+ cod +"'", con.cmdnv);
