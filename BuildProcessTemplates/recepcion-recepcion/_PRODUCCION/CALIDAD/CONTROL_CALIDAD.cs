    ...
    public partial class CONTROL_CALIDAD : Form
    {
        ...
        private void CONTROL_CALIDADcs_Load(object sender, EventArgs e)
        {
           ...
           // orden = Lab_Marcacion.orden_;
            ...
            dtpF.Clear();
            SqlCommand cmdae = new SqlCommand("SELECT [COD_ORDEN] ,[PROCESO], [FECHA_ING]  FROM [LDN].[DETALLES_CALIDAD] WHERE COD_ORDEN = '" + orden + "' ORDER BY PROCESO", cnx.cmdls);
        ...
        private void cargar_etapas()
        {
            ...
            cmd1 = new SqlCommand("SELECT [ID_ETAPA] AS ID,[NOMBRE] AS ETAPA FROM [LDN].[ETAPAS_LAB]", cnx.cmdls);
        ...
        private void descripciones()
        {
            ...
            cmd1 = new SqlCommand("SELECT PED.[COD_ORDEN] ,PED.[CVE_CLIE] ,CLIE.[NOMBRE] ,PED.[PACIENTE] ,CMP.[NUM_CAJA],INV.[CAMPLIB30] AS MATERIAL,PED.[FECHA_IN] ,CMP.[ODES],CMP.[ODCIL],CMP.[OIES],CMP.[OICIL] FROM [LDN].["+ Exte_ +"].[PEDIDO_ENC] AS PED LEFT JOIN [SAE50Empre"+emp_+"].[dbo].[CLIE"+emp_+"] AS CLIE ON PED.[CVE_CLIE] = LTRIM(RTRIM(CLIE.[CLAVE])) LEFT JOIN [LDN].["+ Exte_ +"].[PEDIDO_DET_CMPL] AS CMP ON PED.[COD_ORDEN] = CMP.[COD_ORDEN] LEFT JOIN [LDN].["+ Exte_ +"].[PEDIDO_DET] AS DE ON PED.COD_ORDEN = DE.COD_ORDEN LEFT JOIN [SAE50Empre"+emp_+"].[dbo].[INVE_CLIB"+emp_+"] AS INV ON DE.CVE_ART = LTRIM(RTRIM(INV.CVE_PROD)) COLLATE Latin1_General_BIN where PED.[COD_ORDEN] LIKE '%" + orden + "%'", cnx.cmdls);
            dr1 = cmd1.ExecuteReader();
            while (dr1.Read())
            {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
           ...
           // validacion(); // que se encuentren todos los campos llenos...
            ...
            //modificando en la tabla de bases con el id nuevo de calidad
            insertando_idcalidad(dop);
        ...
        private void codigo_etapa_()
        {
            ...
            SqlCommand cmd = new SqlCommand("SELECT [ID_ETAPA] ,[NOMBRE] FROM [LDN].[ETAPAS_LAB] WHERE [NOMBRE]  = '"+ area_daño + "'", cnx.cmdls);
        ...
        private void cargar_tipos_daños()
        {
            ...
            cmd = new SqlCommand("SELECT [ID_DAÑO] ,[DESC_DAÑO] FROM [LDN].[TIPO_DAÑADO] where [AREA] = '" + id_etapa + "'", cnx.cmdls);
        ...
        private void cargar_puntos()
        {
            ...
            cmd = new SqlCommand("SELECT [SIGLAS] FROM [LDN].[DEFECTOS] WHERE [CLASIFICACION] = '"+ PUNTO +"'", cnx.cmdls);
            dr = cmd.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void punto_desc()
        {
            ...
            cmd = new SqlCommand("SELECT  [ID_DEF] ,[DESCR_DEF] FROM [LDN].[DEFECTOS] WHERE [SIGLAS] = '"+ punto_ +"'", cnx.cmdls);
            dr = cmd.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void cargar_rayones()
        {
            ...
            cmd = new SqlCommand("SELECT [SIGLAS] FROM [LDN].[DEFECTOS] WHERE [CLASIFICACION] = '" + RAYONES + "'", cnx.cmdls);
            dr = cmd.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void rayon_desc()
        {
            ...
            cmd = new SqlCommand("SELECT  [ID_DEF] ,[DESCR_DEF] FROM [LDN].[DEFECTOS] WHERE [SIGLAS] = '" + raya_ + "'", cnx.cmdls);
            dr = cmd.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void cargar_ar()
        {
            ...
            cmd = new SqlCommand("SELECT [SIGLAS] FROM [LDN].[DEFECTOS] WHERE [CLASIFICACION] = '" + AR + "'", cnx.cmdls);
            dr = cmd.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void ar_desc()
        {
            ...
            cmd = new SqlCommand("SELECT  [ID_DEF] ,[DESCR_DEF] FROM [LDN].[DEFECTOS] WHERE [SIGLAS] = '" + ar_ + "'", cnx.cmdls);
            dr = cmd.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void cargar_laca()
        {
            ...
            cmd = new SqlCommand("SELECT [SIGLAS] FROM [LDN].[DEFECTOS] WHERE [CLASIFICACION] = '" + LACA + "'", cnx.cmdls);
            dr = cmd.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void laca_desc()
        {
            ...
            cmd = new SqlCommand("SELECT  [ID_DEF] ,[DESCR_DEF] FROM [LDN].[DEFECTOS] WHERE [SIGLAS] = '" + laca_ + "'", cnx.cmdls);
            dr = cmd.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void codigo_daño()
        {
            ...
            SqlCommand cmd5 = new SqlCommand("SELECT [ID_DAÑO] ,[DESC_DAÑO] FROM [LDN].[TIPO_DAÑADO] WHERE AREA = '" + id_etapa + "' AND [DESC_DAÑO] =  '"+tipo_daño+"'", cnx.cmdls);
        ...
        {
            cnx.conectar("LESA");
            SqlCommand cmd = new SqlCommand("SELECT top 1 [ID_LAB] ,[ID_ORDER] ,[FECHA_ING] ,[REPROCESO] ,[FECHA_REPRO] FROM [LDN].["+Exte_+"].[LABORATORIO_SEG] where [ID_ORDER] = '"+ label7.Text +"' order by [ID_LAB] desc", cnx.cmdls);
            SqlDataReader dr = cmd.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void INSERTANDO_()
        {
            ...
            if (dialogResult == DialogResult.Yes)
            {
                ...
                cmd.Parameters.AddWithValue("@FECHA_ING ", fecha);
                ...
                cmd.Parameters.AddWithValue("@ID_BASE ", id_base);
        ...
        {
            cnx.conectar("LESA");
            SqlCommand cmd = new SqlCommand("SELECT TOP 1 [ID_TRAN] ,[ID_PED] ,[COD_ORDEN] FROM [LDN].[PEDIDO_DET_BASE] WHERE COD_ORDEN = '"+ orden +"' AND ID_PED IS NULL ORDER BY ID_TRAN DESC ", cnx.cmdls);
            SqlDataReader dr = cmd.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void insertando_idcalidad (string orden_)
        {
            util_bas(orden_);

            cnx.conectar("NV");
            SqlCommand cmd = new SqlCommand("UPDATE [LDN].[PEDIDO_DET_BASE] SET [ID_PED] = '" + id_calidad_+ "' WHERE COD_ORDEN = '"+ orden_ +"' AND ID_PED IS NULL  ", cnx.cmdnv);
            cmd.ExecuteNonQuery();
            cnx.Desconectar("NV");

        }
        //codigo de bases
        private void util_bas(string orden)
        {
             cnx.conectar("LESA");
            SqlCommand cmd = new SqlCommand(" SELECT TOP 1 [ID] ,[COD_ORDEN] ,[ID_BASE]  FROM [LDN].[DETALLES_CALIDAD] WHERE COD_ORDEN = '"+ orden+ "' order by ID desc", cnx.cmdls);
            SqlDataReader dr = cmd.ExecuteReader();
            while (dr.Read())
            {
